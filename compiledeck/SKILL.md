...
allowed-tools: Bash(pdflatex*), Bash(latexmk*), Bash(ls*), Bash(open*), Bash(python*), Bash(cp*), Bash(mkdir*), Read, Write, Edit, Glob, Grep
...
### Title capitalization: Chicago-style title case
- **Capitalize**: nouns, verbs, adjectives, adverbs, pronouns, subordinating conjunctions, first and last word
- **Lowercase**: articles (a, an, the), coordinating conjunctions (and, but, or, for, nor, yet, so), prepositions regardless of length (in, on, at, to, by, of, with, from, into, across, beyond, via, between, through, etc.)
- Exception: always capitalize the first and last word of a title
...
### Preferred: Elegant DS Theme (navy/coral/slate)

Always use the **Elegant DS Beamer theme** as the default template. The complete theme is **bundled in this skill directory**:
- `~/.claude/skills/compiledeck/loadslides.tex` — package loader
- `~/.claude/skills/compiledeck/styles/` — all 7 `.sty` theme files

**Features:**
- Professional navy/coral/slate palette with `primary`, `secondary`, `tertiary` color names
- Color aliases: `dsPrimary`, `dsAccent`, `dsSlate`, `dsLight`, `dsWhite`, `dsText`, `dsMid`
- Custom macros: `\hlc{}`, `\hlb{}`, `\cmd{}`, `\file{}`, `\pkg{}`, `\robj{}`, `\promptbox{}`, `\dsnote{}`, `\dstip{}`, `\resetsteps`, `\step{}`, `\key{}`, `\twocol{}{}`
- Code environments: `shellbox` (dark terminal), `rcode` (R code), `codebox` (generic)
- Auto-generated section divider slides via `\AtBeginSection`
- Progress bar footline with frame numbers

**Setup for a new deck:**
1. Copy the theme files into the project:
```bash
mkdir -p temp/styles
cp ~/.claude/skills/compiledeck/loadslides.tex temp/
cp ~/.claude/skills/compiledeck/styles/*.sty temp/styles/
```
2. Use this preamble:
```latex
\documentclass[aspectratio=1610, 11pt]{beamer}
\graphicspath{{figures/}}
\input{temp/loadslides}
```

If the project already has a `temp/` folder with the theme, skip the copy step.

### Fallback: Warm Professional Palette (10 colors)

If for some reason the Elegant DS theme cannot be used, use this self-contained template instead.
...
### 5g: Clean up build artifacts

After you are done compiling and the final PDF is confirmed, move all temporary build files into the project's `temp/` folder. Do not leave `.aux`, `.log`, `.nav`, `.out`, `.snm`, `.toc`, `.vrb`, `.fls`, `.fdb_latexmk`, or any other LaTeX-generated files in the project root. The only files that should remain in the root are the `.tex` source, the `.pdf` output, and project files like `CLAUDE.md`.

```bash
mv [file].aux [file].log [file].nav [file].out [file].snm [file].toc [file].vrb temp/ 2>/dev/null
```
...
- **styles/** — Complete Elegant DS Beamer theme (7 `.sty` files). Copy into project's `temp/styles/`.
- **loadslides.tex** — Package loader for the Elegant DS theme. Copy into project's `temp/`.
